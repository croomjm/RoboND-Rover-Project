...
#define a function to add the rover position to the map as white dot
def plot_rover(pos, plotmap, scale = 10, dot_size = 2):
      y, x = np.int_(pos)
      plotmap[x-dot_size:x+dot_size, y-dot_size:y+dot_size, :] = 255
      return plotmap
...
def update_rover(Rover, data):
      ...
      #print out status messages in 'status' dict
...
def create_output_images(Rover):
      ...
      plotmap[:, :, 0] = obstacle
      plotmap[:, :, 2] = navigable
      #plot the postion of the rover on the map
      plotmap = plot_rover(Rover.pos, plotmap)
      ...
      cv2.putText(map_add,"Mode: "+str(Rover.mode), (0, 70), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.35, (255, 255, 255), 1)
      cv2.putText(map_add,"Action: "+str(Rover.action), (0, 85), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.35, (255, 255, 255), 1)
